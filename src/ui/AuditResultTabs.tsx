...
export default function AuditResultTabs ({
    ...
    // @ts-ignore
    const baseProducts = resultData && baseData ? baseData.map(bd => ({...bd, status: resultData.find(rD => bd['Article/Code barre'] && (bd['Article/Code barre'].toUpperCase().trim() === rD[resultBarcodeColumnName].toUpperCase().trim())) === undefined ? false : true})) : baseData
    // @ts-ignore
