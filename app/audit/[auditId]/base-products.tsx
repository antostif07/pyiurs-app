...
export default async function BaseProducts(props: {
    audit: Audit,
    page?: string,
}) {
    const currentPage = Number(props.page) || 1

    const auditProducts = await apiGetData<AuditProduct>(`/audit_products?auditId=${props.audit.id}&type=base&page=${currentPage}`, 'auditProducts')
